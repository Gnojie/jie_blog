...
Commonjs是一种模块化规范(思想)
Commonjs是一种规范并不是一种具体的代码，类似promise A+规范，只要符合规范里所有的条件，那它就是Commonjs。
...
## 浏览器加载资源
https://npm.devtool.tech/ 可以看各种第三方包的cdn，看看别人的导出方式
控制台直接输入
```js
...
第三方治愈啊Import只能引用完全的 URL，相对以前的裸导入 (bare import specifiers)，很不太方便，如下例:
...
浏览器编译时，处理依赖，先加载文件再编译收集依赖再加载文件...

浏览器加载文件的功能我们称为加载器，加载器开始加载文件时，会把文件路径作为key记录到模块映射(缓存)中
不等文件加载完成，标记为加载中。继续开始下一个文件。
加载完成后触发解析文件，识别静态语法中import。继续触发加载器，以此形成了一种深度遍历的效果，不过是不等待的那种
...
### 代码需要基于es开发
### 第三方资源加载
node_module要可以通过import加载到
加载到的第三方包要兼容esm和cjs(兼容cjs干嘛？)
import CDN地址 的包

### 不支持esm的浏览器
不支持esm的浏览器会跳过type=module的js
这时再html写多一个bundle.js commonjs规范的代码资源即可
浏览器提供一个标签属性`<script nomodule src="">`会判断是否支持esm，不支持加载该标签


[systemjs](https://github.com/systemjs/systemjs)实现
```html
